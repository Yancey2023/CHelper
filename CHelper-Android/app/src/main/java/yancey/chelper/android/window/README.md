...
View 组件。希望以后这个 bug 可以解决，让悬浮窗与 app 界面可以共用一套代码。
...
2. ComposeView 的 context 传入 Application 会导致内嵌 View 作为输入框无法获取焦点，需要改为传入
   Activity 作为 context。
...
2. 输入框获取到焦点后切换页面，获取到焦点的输入框会消失，会导致后续无法获取返回键事件。所以需要在切换页面的时候让
   ...
   因为除了返回上一个页面需要要请求焦点，打开新页面也需要请求焦点，所以我们需要监听 compose
   的导航事件来处理这个问题， 但是目前我没找到相关 API。
