...
1. 增加：命令库支持生成链接分享功能
2. 增加：命令库添加收藏功能
3. 增加：允许直接打开站内信内网址链接并适配
4. 增加：个人中心界面新增创作季相关内容
5. 增加：命令补全页面新增多行编辑模式（长按输入框左侧箭头开关）
6. 增加：设置新增游龙导入时是否自动缩小为小图标
7. 修改：修改了命令库预览界面，将常用内容调整至首页
8. 修改：命令补全页面可直接点击报错内容跳转至对应位置
9. 修改：重做了json生成器相关内容
10. 修改：优化了部分其他内容
11. 修复：修复了部分命令补全相关的bug
...
v0.4.7 20260623
1. 增加：命令库语法高亮（感谢 Lans ）
2. 增加：命令库增加了举报功能（感谢 Lans ）
3. 修改：命令库更新了用户中心（感谢 Lans ）
4. 修改：合并了本地与公有命令库（感谢 Lans ）
5. 修改：优化了命令库搜索体验（感谢 Lans ）
6. 修改：优化了命令库猜你喜欢体验（感谢 Lans ）
7. 修复：命令库编辑器闪退（感谢 Lans ）
8. 修复：资源包已知错误
9. 修复：其它已知问题

v0.4.6 20260502
1. 新增：在线命令库添加了猜你喜欢（感谢 Lans ）
2. 新增：在线命令库添加了站内信功能（感谢 Lans ）
3. 新增：在线命令库添加了游龙导出（感谢 Lans ）
4. 修改：优化了在线命令库编辑体验（感谢 Lans ）
5. 修改：修改了在线命令库部分UI（感谢 Lans ）
6. 修复：修复已知崩溃
7. 修复：修复对话框弹出时界面崩坏

v0.4.5 20260417
1. 新增：为应用引入了完整的命令库云端生态基础设施，包括云函数库广场、创作者主页、排行榜系统、以及对 MCDv2 的全套解析与渲染支持（感谢 Lans ）
2. 新增：支持设置悬浮窗字体是否透明（感谢 Lans ）
3. 新增：优化语法高亮性能，并支持自定义语法高亮字数上限（感谢 Lans ）
4. 修复：恢复上次输入的命令时，如果数据损坏软件会崩溃
...
v0.4.3 20260315
1. 新增：公有命令库开始测试（感谢 Lans ）
2. 修改：把悬浮窗透明度设置选项拆分为悬浮窗图标透明度和悬浮窗界面透明度
3. 修改：命令复制时增加已复制提示
3. 修复：隐私政策弹窗点击窗口外部会导致弹窗消失且界面内容无法点击
4. 修复：屏幕高度不够的时候底部输入框不显示
5. 修复：悬浮窗在关闭显示错误信息的情况仍然显示错误信息
6. 修复：悬浮窗主题不与设置选项同步
7. 修复：部分机型输入框文本显示错位
8. 修复：私有命令库的复制内容与注释内容识别反了
...
v0.4.1 20260314
1. 修复：应用模式跳转界面时输入法不能正常隐藏
2. 修复：悬浮窗模式输入法弹出时界面不能正常收缩
3. 修复：悬浮窗模式无法显示弹窗

v0.4.0 20260313
1. 重构：使用 Jetpack Compose 框架进行重构，90% 的代码已经被重写。我们尽量还原了之前的软件界面和操作逻辑，以减少本次更新对用户的影响。
2. 修改：内置资源包适配最新版 MC（感谢 @小白子 ）
